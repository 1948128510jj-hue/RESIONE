      ...
      <div className="container-wide flex items-center justify-between h-16 sm:h-[72px]">
        ...
        <nav className="hidden lg:flex items-center gap-0.5">
          {SITE_CONFIG.nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="px-3.5 py-2 text-sm font-medium text-foreground/70 hover:text-primary hover:bg-surface rounded-md transition-colors"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <a
            ...
            className="hidden sm:flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors"
          >
