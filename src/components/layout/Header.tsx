    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-border shadow-sm">
      <div className="container-wide flex items-center justify-between h-16">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {SITE_CONFIG.nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="px-3 py-2 text-sm font-medium text-foreground/80 hover:text-primary hover:bg-surface rounded-md transition-colors"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          {/* WhatsApp quick contact - most important for closing */}
          <a
            href={`https://wa.me/${SITE_CONFIG.whatsapp.replace(/[^0-9]/g, '')}`}
            target="_blank"
            rel="noopener noreferrer"
            className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors"
          >
          ...
          <button className="lg:hidden p-2 text-foreground hover:bg-surface rounded-md" aria-label="Menu">
